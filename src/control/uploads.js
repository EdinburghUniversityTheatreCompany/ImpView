...
async function renderRow(kind) {
  ...
  for (const record of records) {
    ...
    if (kind === 'image') {
      ...
      a.appendChild(video);
      setTimeout(() => {
        try { video.currentTime = 2; } catch (_) { /* metadata not loaded yet */ }
      }, 1000);
    }
    ...
    a.addEventListener('click', (e) => {
      ...
      if (kind === 'video') {
        document.querySelectorAll('.preset-videos a.active').forEach((t) => t.classList.remove('active'));
      }
  ...
  fileInput.addEventListener('change', async () => {
    ...
    for (const file of files) {
      try {
        ...
        if (result.added) added++; else dupes++;
      } catch (err) {
