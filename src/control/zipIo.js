...
async function exportZip() {
  ...
  if (!records.length) {
    showModal({ title: 'Export ZIP', bodyHtml: '<p>Your library is empty — nothing to export.</p>' });
    return;
  }
...
async function importZip(file) {
  ...
  for (const [path, bytes] of Object.entries(entries)) {
    ...
    let kind = null;
    let name = null;
    if (path.startsWith('images/')) { kind = 'image'; name = path.slice('images/'.length); }
    else if (path.startsWith('videos/')) { kind = 'video'; name = path.slice('videos/'.length); }
  ...
  if (errors.length) {
    parts.push(`<p><strong>Errors:</strong></p><pre>${errors.map((e) => e.replace(/</g, '&lt;')).join('\n')}</pre>`);
  }
