...
function wrap(nodes) {
  if (!nodes) nodes = [];
  if (nodes instanceof Element || nodes instanceof Document || nodes instanceof Window)
    nodes = [nodes];
  ...
  const w = {
    ...
    css(prop, value) {
      if (value === undefined)
        return nodes[0] ? getComputedStyle(nodes[0]).getPropertyValue(prop) : "";
      nodes.forEach((n) => {
        n.style[prop] = value;
      });
      return w;
    ...
    append(htmlOrNode) {
      if (typeof htmlOrNode === "string") {
        nodes.forEach((n) => {
          n.insertAdjacentHTML("beforeend", htmlOrNode);
        });
      } else {
    ...
    data(key, value) {
      ...
      nodes.forEach((n) => {
        n.dataset[key.replace(/-([a-z])/g, (_, c) => c.toUpperCase())] = value;
      });
      return w;
    },
    closest(selector) {
